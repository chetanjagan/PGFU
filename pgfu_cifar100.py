...
pgfu_cifar100.py
================
PGFU Unlearning — CIFAR-100

Teacher : ResNet34 (pretrained on full CIFAR-100)
Student : ResNet18 (trained via PGFU)
...
Usage:
    ...
    for seed in 42 123 456; do
        python pgfu_cifar100.py --seed $seed \
            --teacher_path checkpoints/teacher_cifar100_resnet34.pth \
...
from linear_probe import run_probe, get_data


# ── Configuration ─────────────────────────────────────────────────────────
def parse_args():
    p = argparse.ArgumentParser(description="PGFU CIFAR-100 Unlearning")
    p.add_argument("--teacher_path",   type=str,   required=True)
    p.add_argument("--results_dir",    type=str,   default="results/cifar100")
...
def get_cifar100_loaders(args):
    ...
    trainset = torchvision.datasets.CIFAR100(
        args.data_dir, True,  download=True, transform=transform_train)
    testset  = torchvision.datasets.CIFAR100(
        args.data_dir, False, download=True, transform=transform_test)

    train_labels = torch.tensor(trainset.targets)
    test_labels  = torch.tensor(testset.targets)
    forget_set   = set(args.forget_classes)

    forget_idx      = [i for i, l in enumerate(train_labels) if l.item() in forget_set]
    keep_idx        = [i for i, l in enumerate(train_labels) if l.item() not in forget_set]
    ...
    return (keep_loader, forget_loader,
            ...
            keep_idx, train_labels)


# ── Prototype Computation ─────────────────────────────────────────────────
def compute_prototypes(teacher, keep_idx, train_labels, trainset_raw, device):
    print("Computing confidence-weighted prototypes...", flush=True)
    teacher.eval()
    hook = FeatureHook(teacher.layer4)

    raw_imgs = trainset_raw.data        # numpy [N, 32, 32, 3]
    raw_labs = np.array(trainset_raw.targets)
    imgs     = raw_imgs[keep_idx]
    labs     = raw_labs[keep_idx]
    N        = len(imgs)

    # Probe feature dimension
    with torch.no_grad():
        s  = (torch.from_numpy(imgs[:1].copy()).float() / 255.0).permute(0,3,1,2)
        s  = (s - _MEAN_T) / _STD_T
        teacher(s.to(device))
    ...
    MINI = 32
    with torch.no_grad():
        for bi in tqdm(range((N + MINI - 1) // MINI),
                       desc="  Prototypes", leave=False):
            s  = bi * MINI; e = min(s + MINI, N)
            x  = (torch.from_numpy(imgs[s:e].copy()).float() / 255.0).permute(0,3,1,2)
            x  = (x - _MEAN_T) / _STD_T
            l  = torch.from_numpy(labs[s:e].copy()).long()
    ...
    print(f"  Done. {len(protos)} prototypes computed.", flush=True)
    return protos, gp_vec


# ── Stage 1 ───────────────────────────────────────────────────────────────
def train_stage1(args, keep_loader, device):
    print("\nStage 1: Retain-Set Pretraining...", flush=True)
    ...
    for epoch in range(1, args.stage1_epochs + 1):
        model.train(); tloss = cor = tot = 0
        for x, y in tqdm(keep_loader, desc=f"  S1 {epoch}/{args.stage1_epochs}",
                         leave=False):
...
def train_stage2(args, student, teacher, keep_loader, forget_loader,
    ...
    with torch.no_grad():
        ...
        teacher(dummy)
        tch, th, tw = (t_hook.features.size(1), t_hook.features.size(2),
                       t_hook.features.size(3))
        student(dummy); sch_dim = s_hook.features.size(1)
        del dummy
    ...
    best_acc = 0.0; best_student = None; best_ema = None
    ...
    for epoch in range(1, args.stage2_epochs + 1):
        ...
        for _ in tqdm(range(steps), desc=f"  S2 {epoch}/{args.stage2_epochs}",
                      leave=False):
            try:    xk, yk = next(k_it)
            ...
            with torch.no_grad():
                tlf     = teacher(xf)
                tf_flat = F.normalize(
                    t_hook.features.view(xf.size(0), -1).detach(), dim=1)
            ...
            gp_val = gradient_penalty(critic, tf_flat.detach(),
                                      sf_flat.detach(), device)
            (lc + args.gp_weight * gp_val).backward()
            opt_c.step()
...
def main():
    ...
    print(f"PGFU — CIFAR-100 | Forget: {args.forget_classes} | Seed: {args.seed}")
    t_start = time.time()

    # Data
    (keep_loader, forget_loader,
     test_keep_loader, test_forget_loader,
     keep_idx, train_labels) = get_cifar100_loaders(args)

    trainset_raw = torchvision.datasets.CIFAR100(
        args.data_dir, True, download=True, transform=None)
    ...
    if os.path.exists(proto_path):
        ...
        prototypes, global_proto = compute_prototypes(
            teacher, keep_idx, train_labels, trainset_raw, device)
    ...
    print(f"\n{'='*55}")
    print(f"  PGFU CIFAR-100 Results (seed={args.seed})")
    print(f"{'='*55}")
    print(f"  Keep Acc   : {keep_acc:.2f}%")
    print(f"  Forget Acc : {forget_acc:.2f}%")
    print(f"  MIA-loss   : {mia_loss:.4f}")
    print(f"  MIA-conf   : {mia_conf:.4f}")
